...
sudo rm -rf /tmp/libyang             # KEY FIX: always clean
cd /tmp
...
echo "=== Step 4: Protobuf and ZeroMQ ==="
sudo apt-get install -y protobuf-c-compiler libprotobuf-c-dev libzmq5 libzmq3-dev

echo "=== Step 5: Build rtrlib v0.6.3 ==="
sudo apt-get install -y libssh-dev
sudo rm -rf /tmp/rtrlib              # KEY FIX: always clean
cd /tmp
git clone https://github.com/rtrlib/rtrlib.git
cd rtrlib                            # KEY FIX: cd BEFORE checkout
git checkout v0.6.3                  # KEY FIX: now runs in correct directory
mkdir build && cd build
...
echo "=== Step 6: FRR users and groups ==="
...
sudo rm -rf /tmp/frr                 # KEY FIX: always clean
cd /tmp
...
# KEY FIX: patch source BEFORE configure/make so Clippy succeeds normally
python3 << 'PYEOF'
import re
filepath = '/tmp/frr/bgpd/bgp_bfd.c'
with open(filepath, 'r') as f:
...
print(f"Fixed {before - after} Clippy-incompatible DEFUN blocks")
with open(filepath, 'w') as f:
    f.write(content)
PYEOF
...
echo "=== Step 10: Build and install ==="
make                                 # KEY FIX: no CLIPPY=: needed
sudo make install
...
echo "=== Step 13: Enable BGP (without -M rpki) ==="
sudo sed -i "/bgpd=no/ cbgpd=yes" /etc/frr/daemons
# KEY FIX: do NOT add -M rpki — module not present causes bgpd crash
sudo sed -i \
  's/bgpd_options="   -A 127.0.0.1 -M rpki"/bgpd_options="   -A 127.0.0.1"/' \
  /etc/frr/daemons 2>/dev/null || true

echo "=== Step 14: Start FRR ==="
